...
/**
 * Upload multiple images to SupabaseAdmin Storage and save to database
 */
...
console.log(files);
  ...
  const { data: insertedImages, error: dbErr } = await supabaseAdmin
    .from("product_images")
    .insert(
...
export async function replaceImage(userId, imageId, newFile) {
  ...
  const { data: existingImage, error: fetchErr } = await supabaseAdmin
    .from("product_images")
    .select("image_url, product_id, is_thumbnail")
    .eq("id", imageId)
    .single();
  ...
  const { data: updatedImage, error: updateErr } = await supabaseAdmin
    .from("product_images")
...
async function setFirstImageAsThumbnail(productId) {
  const { data: remainingImages, error: fetchErr } = await supabaseAdmin
    .from("product_images")
    .select("image_url")
    .eq("product_id", productId)
    .limit(1);
