    ...
    async (payload) => {
      ...
      try {
        ...
        if (err instanceof ApiError && err.isValidation) {
          ...
          toast.error(
            err instanceof ApiError ? err.message : "Could not update store."
          );
        }
        return null;
      } finally {
...
export function useCreateLocation() {
  const storeId = useAuthStore((s) => s.storeId);
  const _init = useAuthStore((s) => s.init)
  const [loading, setLoad] = useState(false);
  ...
  const create = useCallback(
    async (payload) => {
      ...
      try {
        ...
        toast.error(
          err instanceof ApiError ? err.message : "Could not add location."
        );
        return null;
      } finally {
        _init();
...
export function useUpdateLocation() {
  ...
  const update = useCallback(
    async (locationId, payload) => {
      ...
      setLoad(true);
      try {
        const result = await storeService.updateLocation(storeId,locationId, payload);
  ...
  const deleteLocation = useCallback(
    async (locationId) => {
      ...
      try {
        ...
        toast.error(
          err instanceof ApiError ? err.message : "Could not remove location."
        );
        return false;
      } finally {
        setLoad(false);
