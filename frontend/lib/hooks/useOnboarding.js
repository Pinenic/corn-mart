...
export function useUploadStoreAssets() {
  ...
  const upload = useCallback(async (storeId, { logoFile, bannerFile }) => {
    ...
    const doUpload = async (file, field) => {
      ...
      const res = await fetch(`${baseUrl}/stores/${storeId}/assets/${field}`, {
        method: "PATCH",
        headers: token ? { Authorization: `Bearer ${token}` } : {},
        body: fd,
      });
      const json = await res.json();
      if (res.ok && json.success) return json.data?.[field] ?? null;
      return null;
    };
    ...
    try {
      ...
      /* silent — store was created, assets are optional */
    }
  ...
  const create = useCallback(async (storeId, { details, imageFiles }) => {
    ...
    try {
      ...
      if (imageFiles?.length > 0) {
        ...
        fd.append("variant_id", product.variants[0].id);

        await fetch(
          `${baseUrl}/stores/${storeId}/products/${product.id}/images`,
          {
            method: "POST",
            headers: token ? { Authorization: `Bearer ${token}` } : {},
            body: fd,
