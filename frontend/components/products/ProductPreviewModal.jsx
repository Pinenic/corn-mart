...
export function ProductPreviewModal({ product: initial, open, onClose }) {
  ...
  const handleChat = async () => {
    setMessageRef({
      productId: full.id,
      image_url: full.thumbnail_url,
      name: full.name,
      link: `/marketplace/products/${full.id}`,
    });
      ...
      {isLoading && !full ? (
        ...
        <div className="flex flex-col md:flex-row gap-0">
          ...
          <div className="md:w-1/2 flex flex-col p-5 gap-4">
            {full.store && (
              <Link
                href={`/marketplace/stores/${full.store_id}`}
            ...
            <div>
              <p className="text-[18px] font-bold text-[var(--color-text-primary)] leading-snug">
                {full.name}
              </p>
              {full.brand && (
            ...
            {!outOfStock && stock >= 5 && (
              <p className="text-[12px] font-medium text-[var(--color-info)]">
                {" "}
                {stock} available
              </p>
            )}
            ...
            {/* Qty + Add */}
            <div className="grid grid-cols-2 items-center gap-3 mt-auto pt-2">
              {/* Qty picker */}
              <div className="flex items-center justify-evenly border border-[var(--color-border-md)] rounded-xl overflow-hidden">
                <button
                  onClick={() => setQty((q) => Math.max(1, q - 1))}
                  className="w-9 h-9 flex items-center justify-center hover:bg-[var(--color-bg)] text-[var(--color-text-secondary)] font-bold"
                >
                  −
                </button>
            ...
            {/* View full page link */}
            <Link
              href={`/marketplace/products/${full.id}`}
              className="flex items-center gap-1 text-[12px] text-[var(--color-text-muted)] hover:text-[var(--color-primary)] transition-colors"
              onClick={onClose}
            >
