...
import {
  useCreateStore,
  useUploadStoreAssets,
  useCreateFirstProduct,
  ...
  return (
    <div className="h-1 w-full bg-[var(--color-border)] rounded-full overflow-hidden">
      <div
        ...
        style={{
          width: `${pct}%`,
          background: STEPS[step]?.color ?? "#0057ff",
        }}
      />
    </div>
  );
          ...
          <div key={s.key} className="flex items-center gap-2">
            ...
            {i < STEPS.length - 1 && (
              <div
                ...
                style={{
                  background:
                    i < current ? "#16a34a40" : "var(--color-border-md)",
                }}
              />
            )}
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl">
        <button
          onClick={onCancel}
          className="absolute top-4 right-4 w-7 h-7 flex items-center justify-center rounded-xl hover:bg-[var(--color-bg)]"
        >
        ...
        >
          Your store is live! You can always add products later from your
          dashboard. Stores with products get 3× more views.
        </p>
...
export default function OnboardingPage() {
  ...
  const {
    create: createStore,
    loading: storelLoading,
    fieldErrors: storeErrors,
  } = useCreateStore();
  const { upload: uploadAssets, loading: assetsLoading } =
    useUploadStoreAssets();
  const { data: categoriesRaw } = useCategories();
  const { create: createProduct, loading: productLoading } =
    useCreateFirstProduct();
  ...
  const validateDetails = () => {
    ...
    if (detailsForm.name.trim().length < 2)
  ...
  const canAdvance = () => {
    ...
    if (step === 2)
      return productForm.name.trim() && Number(productForm.price) > 0;
    return true;
  };
  ...
  const primaryLabel = () => {
    ...
    if (step === 0)
      return (
        <>
          Create my store <ArrowRight size={15} />
        </>
      );
    if (step === 1)
      return (
        <>
          Looks great — continue <ArrowRight size={15} />
        </>
      );
    if (step === 2)
      return (
        <>
          Publish &amp; go to dashboard <ArrowRight size={15} />
        </>
      );
  };
    ...
    >
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-6">
          ...
          <div className="flex items-center justify-between h-14 gap-4">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <span
                className="text-[14px] font-bold hidden sm:block"
                style={{ color: "var(--color-text-primary)" }}
              >
                Corn Mart
              </span>
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 md:px-6 py-8 md:py-12">
        ...
        <div className="mb-8 md:mb-10">
          ...
          >
            {step === 0 &&
              "Choose a name and style that reflects what you sell"}
            {step === 1 &&
              "This is exactly how buyers will discover you on the marketplace"}
        ...
        >
          ...
          {step === 1 && <StepPreview store={createdStore} />}
          {step === 2 && (
            <StepProduct
              form={productForm}
      ...
      <div className="sticky bottom-0 z-30 border-t border-[var(--color-border)] bg-white/95 backdrop-blur">
        <div className="max-w-4xl mx-auto px-4 md:px-6 py-4 flex items-center justify-between gap-4">
          ...
          >
            ...
            {step === 2 &&
              "You can add more products, variants and images from the products page"}
          </p>
