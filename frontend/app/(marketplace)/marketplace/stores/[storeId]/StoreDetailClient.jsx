...
import { parseStoreConfig }    from "@/lib/storefront/configSchema";
...
export function StoreDetailClient({ storeId }) {
  ...
  const goToPage = (p) => {
    router.replace(`${pathname}?${buildParams(searchParams, { page: p === 1 ? null : p })}`, { scroll: false });
  };
  ...
  // Falls back to a sane default (hero + New Arrival tab) for the many
  // stores that haven't customized config yet — see
  // lib/storefront/configSchema.js
  const blocks = parseStoreConfig(store.config);
    ...
    <div>
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 pb-12">
        ...
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-[18px] font-bold text-[var(--color-text-primary)]">All Products</h2>
        ...
        {meta && meta.totalPages > 1 && (
          <div className="flex justify-center gap-2 mt-10">
            <Button variant="secondary" size="sm" disabled={page === 1} onClick={() => goToPage(page - 1)}>‹ Prev</Button>
            <span className="text-[13px] text-[var(--color-text-secondary)] px-2">Page {page} of {meta.totalPages}</span>
            <Button variant="secondary" size="sm" disabled={page >= meta.totalPages} onClick={() => goToPage(page + 1)}>Next ›</Button>
          </div>
        )}
      </div>
