...
const PERKS = [
  {
    ...
    desc: "Every store is reviewed before going live",
  },
  {
    icon: MessageCircle,
  ...
  },
  {
    ...
    desc: "Sellers offer multiple delivery options",
  },
  {
    icon: Sparkles,
    title: "Curated products",
    desc: "Quality goods from independent creators",
  },
];
...
export default function HomePage() {
  ...
  const { products, isLoading: pLoading } = useMarketplaceProducts({
    limit: 8,
    sort: "created_at",
    order: "desc",
  });
    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[var(--color-primary)] via-blue-600 to-indigo-700 text-white">
        ...
        <div className="relative max-w-5xl mx-auto px-4 md:px-6 py-20 md:py-28 text-center">
          ...
          <p className="text-[16px] md:text-[18px] text-blue-100 max-w-xl mx-auto mb-8 leading-relaxed">
            Browse thousands of products from independent sellers. Find unique
            items and support local businesses.
          </p>
          <div className="flex flex-wrap gap-3 justify-center">
            <Link href="/marketplace/products">
              <Button
                size="lg"
                className="bg-white text-[var(--color-primary)] hover:bg-blue-50 font-bold shadow-lg"
              >
        ...
        <div className="max-w-5xl mx-auto px-4 md:px-6 py-4">
          ...
          >
            {CATEGORIES.map((cat) => (
              <Link
                key={cat.name}
                href={`/marketplace/products?category=${encodeURIComponent(
                  cat.name
                )}`}
              >
                <div className="flex items-center gap-2 px-4 py-2 rounded-xl border border-[var(--color-border)] bg-white hover:border-[var(--color-primary)] hover:bg-[var(--color-primary-light)] transition-all duration-200 cursor-pointer flex-shrink-0 group">
                  ...
                  <span className="text-[12px] font-semibold text-[var(--color-text-secondary)] group-hover:text-[var(--color-primary-text)] whitespace-nowrap">
                    {cat.name}
                  </span>
                </div>
              </Link>
      ...
      <section className="max-w-5xl mx-auto px-4 md:px-6 py-12">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-[13px] text-[var(--color-text-secondary)] mt-0.5">
              Shops people keep coming back to
            </p>
          </div>
          <Link
      ...
      <section className="max-w-5xl mx-auto px-4 md:px-6 pb-16">
        <div className="bg-gradient-to-r from-[var(--color-primary)] to-indigo-600 rounded-3xl p-8 md:p-12 text-white text-center">
          <h2 className="text-[24px] md:text-[30px] font-bold mb-3">
            Have a store?
          </h2>
          <p className="text-blue-100 text-[14px] mb-6 max-w-md mx-auto">
            List your products, reach thousands of buyers and manage everything
            from one dashboard.
          </p>
